...
def get_file(filesuffix: str,callback):
    layout = [[sg.Text('选择原始文件')], [sg.Input(key='fileName',enable_events=True), sg.FileBrowse('打开',file_types=(("Text Files", f"*.{filesuffix}*"),))],
              [sg.Button('   开始处理   '), sg.Button('      关闭      ')]]
    ...
    while True:
        ...
        if event == '   开始处理   ':
            ...
            if os.path.exists(fileName):
                ...
                sg.popup('请选择正确的待处理文件')
    window.close()
    
def get_saveFilePath(callback):
    layout = [[sg.Text('选择结果存储地址')], [sg.Input(key='path',enable_events=True), sg.FolderBrowse('浏览')],
          [sg.Button('   开始处理   '), sg.Button('      关闭      ')]]
    window = sg.Window('解析工具', layout)
    while True:
